...
class Critic(nn.Module):
	...
	def forward(self, state, action, record_without=False):
		...
		if record_without:
			...
			q2 = F.relu(self.l5(q2))
			q2 = self.l6(q2)
			return q1_noise, q2_noise, q1, q2
		return q1, q2
...
class GRAC():
	...
	def train(self, replay_buffer, batch_size=100, writer=None, reward_range=20.0, **reward_kwargs):
		...
		if self.total_it % 1 == 0:
			...
			q1_actor_action_noise, q2_actor_action_noise, q1_actor_action, q2_actor_action = self.critic(state, actor_action, record_without=True)
			q1_noise_diff = q1_actor_action_noise - q1_actor_action
			q2_noise_diff = q2_actor_action_noise - q2_actor_action
		...
		if log_it:
			with torch.no_grad():
				...
				writer.add_scalar('train_critic/Q1_noise_diff', q1_noise_diff.abs().mean(), self.total_it)
				writer.add_scalar('train_critic/Q2_noise_diff', q2_noise_diff.abs().mean(), self.total_it)
