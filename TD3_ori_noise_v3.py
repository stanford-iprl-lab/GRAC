		...
		if self.noise_flag:
			with torch.no_grad():
				for param in self.l6.parameters():
					param.add_(torch.randn(param.size(), device=self.device) * param * self.noise_percent)
...
class GRAC(object):
	...
	def train(self, replay_buffer, batch_size=100, writer=None, reward_range=20.0):
		...
		self.critic_target.noise_flag = False
		with torch.no_grad():
		...
		if self.total_it % self.policy_freq == 0:
			...
			self.critic_target.noise_flag = True
			self.critic.add_noise_nn_weights()
			self.critic_target.add_noise_nn_weights()
